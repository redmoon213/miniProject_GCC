...
- **예시 코드 제공:** 사용자가 명시적인 수정을 요구하지 않은 경우(Inquiry), 파일 시스템을 수정하는 대신 프롬프트 상에서만 참고용 예시 코드를 출력합니다.
...
- **기존 코드 존중:** 코드를 수정하거나 기능을 추가할 때, 사용자가 이미 작성한 로직과 구조를 최대한 존중하며 그 사이에 새로운 코드를 안전하게 삽입(Surgical Update)하는 것을 원칙으로 합니다. 불필요한 전체 재작성이나 구조 변경을 지양합니다.
- **기존 주석 유지:** 코드 수정 시 기존에 주석 처리되어 있는 코드는 문맥상 어색하더라도 사용자의 명시적 요청이 없는 한 절대로 임의로 주석을 해제하지 않습니다.
